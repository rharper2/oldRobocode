...
#include "spatial.h"
#include "spatialRobocodeCreatureTheta.h"

// Some typedefs to save myself typing and debugging the inevitable spelling msitakes.
typedef spatialWorld<spatialRobocodeCreature,spatialRobocodeParasite> theWorld;
...
void saveEm(theWorld *thePtr) {
	...
	vector<boost::shared_ptr<location<spatialRobocodeCreature,spatialRobocodeParasite> > >::iterator vat,vend;
	...
	while (vat != thePtr->theWorld.end()) {
		...
		if ((*vat)->creature->getCreature()->isValid()) {
			...
			ofstream robotout;
			robotout.open(jname);
			robocodeTheta::Instance()->print_grammar_individual(robotout,(*vat)->creature->getCreature()->getExpression(),(*vat)->creature->getCreature()->getExpressionLength(),fname,gname);
			robotout.close();
		} else {
		...
		if ((*vat)->parasite->getParasite()->isValid()) {
			...
			ofstream robotout;
			robotout.open(jname);
			robocodeTheta::Instance()->print_grammar_individual(robotout,(*vat)->parasite->getParasite()->getExpression(),(*vat)->parasite->getParasite()->getExpressionLength(),fname,gname);
			robotout.close();
		} else {
...
void doit() {
 ...
 // This is the file the population is saved at, (in each second generation folder)
	const char *fileToUse = "TestOneRoboCodeTheta";

	
	robocodeTheta *grammar = robocodeTheta::Instance();
