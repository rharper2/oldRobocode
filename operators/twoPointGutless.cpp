...
/*******************************************************************
                    TWO POINT GUTLESS
                    Like two point codon, except the middle segment (the gut)
                    is filled with random bits. Again I find this strangely familiar on a personal level
********************************************************************/
void cr_data::twoPointGutlessOperator(const crPtr &op1,const crPtr &op2,crPtr &child1,crPtr &child2) {
 ...
 // okay the + is only at the CODON boundaries.
          ...
          do {
                    ...
                    if (valid) {
                              ...
                              for (i=0;i<slice1;i++) 
                                        newDna[i]=pdna1[i];
                              for (i=0;i<chunk2;i++)
                                        newDna[slice1+i]=randomint(2); // ie zero or 1
                              for (i=0;i<offset1;i++) 
   ...
   //                     invalid_count++;
                              ...
                              if (thisInvalidCount>5) {
                                        invalid_count++;
    //                   cout << "Giving up, last reason it was invalid was " << grammar->getInvalidReason() << "\n";
                              }
                    }
