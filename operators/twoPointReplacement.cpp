...
void cr_data::twoPointReplacementOperator(const crPtr &op1,const crPtr &op2,crPtr &child1,crPtr &child2) {
    ...
    do {
        ...
        if (expr_to_match == 0 && len1 <= 2) {
            // what we basically have is a single cvalue - we cant do much with that.
...
//        cout << "Failed to find a non-leaf node\n";
 ...
 //   int matched;
    ...
    if (found > 0) {
        ...
        int endOfExp2 = ((codons1[found] >> (CODON + LHSINFO)) & EXPRMASK)+c_slice2;
        ...
        if (fullDnaToBeUsed) { 
            ...
            // Left for historical interest I cant see why one would not allow fullDnaToBeUsed.
...
//            cout << "And after... " << part1c << endl;
   ...
   /*         if (!child1->isValid()) { // Note that with some grammars it is possible to have invalid children 
                                        // If a "body" is sliced into a "function" it might cause a "ripple" which invalidates the child
                                        // This is nothing to be worried about (although too many invalid children will slow things down.
