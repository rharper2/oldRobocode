        ...
        if (((codons1[i] >> CODON ) & LHSMASK) == matched) {
            if (secondFindNonTerminal) {
                if (!findNonTerminal) { // i.e. we have a terminal on the otherside
                    ...
                    if (gb->isNonTerminal(&codons1[i]))  secondNTPos.push_back(i);
                }
            }
            else { 
...
//        cout << "========================================================================\n";
    ...
    if (secondFindNonTerminal) {
        ...
        if (size ==0) {
            return; // just abandon attempt we have a leaf entity.
    ...
    // so we just use the code we have used before to do this
	long p1,p2,p3;
    ...
    if (c_slice2 >= 0) {
        ...
        */
		...
		// extract the length (and therefore the end) of the expression from the codon info
        ...
        long part1b = (endOfExp2 - c_slice2) * CODON; // the  of the expression to be inserted
        long part1c;
        long part1cAlt;
		...
		if (fullDnaToBeUsed) { 
            ...
            if (part1cAlt < 0) {
				// PRINT OUT SOME ERROR DIAGNOSITCS FOR US
				cout << "Error we have a negative part1cAlt\n";
				  cout << "C_slice 1 is:                 " << c_slice1 << endl;
				cout << "Part1a (c_slice1*CODON) is:   " << c_slice1*CODON << endl;
				long length = (codons0[c_slice1] >> (CODON +LHSINFO) & EXPRMASK);
				cout << "Length extracted from c_slice1 is " << length << " which * CODON is " << length*CODON << endl;
				cout << "EndofExp1 = " << endOfExp1 << "End of Exp2 " << endOfExp2 << endl;
				cout << "EndofExp1(Codons) = " << endOfExp1* CODON << "End of Exp2(Codons) " << endOfExp2* CODON << endl;
				cout << "Dna lenght " << pge0->getDnaLength() << "Useful dna is " << pge0->getUsefulDna() << endl;
				cout << "Part1c is " << part1c << "part1cAlt " << part1cAlt<<endl; 
        ...
        if (newDnaLength > (INVALIDTHRESHOLD)) { // we can rebuild it (perhaps!)
            ...
            if ((part1c - (part1cAlt+100)) > excessToTrim) { // ok so we have the room to trim it.
                newDnaLength -= excessToTrim; // so we dont need to copy all of the tail over.
                part1c -= excessToTrim;
			    //cout << "We trimmed it -  part1cAlt is " << part1cAlt << " and part1c is now " << part1c << endl;
				didTrim = true;
            }
        }
        ...
        if (fullDnaToBeUsed) { 
            ...
            if (part2c < 0) {
				cout << "Error we have a negative part2c\n";
			}
        } else { 
        ...
        if (newDnaLength2 > (INVALIDTHRESHOLD)) { // we can rebuild it (perhaps!)
            ...
            // high and also that all the dna will quickly become sized at invalid threshold
            // need to keep an eye on this.
            if ((part2c - (part2cAlt+100)) > excessToTrim) { // ok so we have the room to trim it.
                newDnaLength2 -= excessToTrim; // so we dont need to copy all of the tail over.
                part2c -= excessToTrim;
				didTrim2 = true;
			    //cout << "We trimmed it part2cAlt is " << part2cAlt << " and part2c is now " << part2c << endl;
            } 
        }
...
//            cout << "part2a is " << part2a << endl;
           ...
           // for (int i=15*12;i<16*12;i++) cout << pdna2[i];
           //     cout << endl;
           // for (int i=15*12;i<16*12;i++) cout << newDna2[i];
           //     cout << endl;
...
/*
	// This following block can be used to check that valid children are being produced.
	// Note there are a number of reasons why we might get invalid children, including
	// the fact that the phenotype might be too large.
	
    if (child1 && child2) {
        if (!child1->isValid()) {
				cout << "Child1 was not valid?\n";
				child1->reEvaluate();
				if (!child1->isValid())
				cout << "It was not valid , reason: " << child1->getInvalidReason()  << endl;
				else cout << "It suddenly became valid - freaky!\n";
        ...
        // pge1->resetChanged();
        if (!child2->isValid()) {
           cout << "Child2 was not valid.\n";
			child2->reEvaluate();
			if (!child2->isValid())
			cout << "It was not valid , reason: " << child2->getInvalidReason()  << endl;
			else cout << "It suddenly became valid - freaky!\n";
				
						   // pge0->resetChanged();
     //       if (!pge0->validate(gb)) {
     //           cout << "Hey neither was parent0!\n";
     //       } else cout << "Parent0 looks ok\n";
     //       if (!pge1->validate(gb)) {
     //           cout << "Hey neither was parent1!\n";
     //       } else cout << "Parent1 looks ok\n";
     //        cout << "Error not valid: " << gb->getInvalidReason() << "\n";
  	//		cout << "*****************\n";
	//		if (didTrim2) cout << "Did trim2\n"; else cout << "Did not trim2\n";
     //       cout << "Following: " << endl;
     //       cout << pge0->display(gb);
      //      cout << pge1->display(gb);
