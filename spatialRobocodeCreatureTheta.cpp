...
spatialRobocodeCreature::spatialRobocodeCreature() {
	...
	ofstream robotout;
	robotout.open(jname);
	robocodeTheta::Instance()->print_grammar_individual(robotout,getCreature()->getExpression(),getCreature()->getExpressionLength(),fname,gname);
	robotout.close();
}
						...
						if (nread == 0) {
							...
							{ // okay looks like we have a score
								if ((found = sockfdToCritter.find(fd)) == sockfdToCritter.end()) {
									...
									double totalScore = creatureScore + parasiteScore;
									if (battleLineResult->location == -1) {
										battleLineResult->creature->addToLayeredScore((creatureScore*100)/totalScore);
										battleLineResult->parasite->addToLayeredScore((parasiteScore*100)/totalScore);
										battlesSoFar++;
									} else {
										battleLineResult->creature->addToScore((creatureScore*100)/totalScore);
...
void spatialRobocodeCreature::loadCreature(ifstream &fin) {
	...
	int *dna;
	fin >> geneticAge; 
	cout << "Creature Age:" << geneticAge << endl;
	fin >> dnaL;
	if (!fin) return; 
...
spatialRobocodeParasite::spatialRobocodeParasite() {
	...
	ofstream robotout;
	robotout.open(jname);
	robocodeTheta::Instance()->print_grammar_individual(robotout,getParasite()->getExpression(),getParasite()->getExpressionLength(),fname,gname);
	robotout.close();
...
void spatialRobocodeParasite::doMutation(int i) {
...
{
	streamout(fout,parasite,geneticAge);
}


void spatialRobocodeParasite::loadParasite(ifstream &fin,int i) {
	long dnaL;
	int *dna;
	fin >> geneticAge; 
	cout << "Parasite Age:" << geneticAge << endl;
	fin >> dnaL;
	if (!fin) return; 
	if (dnaL<10 || dnaL>800000) {
		cout << "Encountered an unusual dna length of " << dnaL << "\n";
