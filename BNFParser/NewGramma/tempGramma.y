...
void identifyGrammarRules() {
    ...
    if (funcCode == -1) printf("No functions appear to have been defined\n");
	defunCode = retrieveCodeByName("DEFUN");
	if (defunCode == -1) printf("No function definition appears to have been allowed e.g. DEFUN\n");
	paramCode = retrieveCodeByName("PARAM");
	if (paramCode == -1) printf("No PARAM keyword appears to have been used.\n");
...
int main(int argc, char **argv){
	char text[255], text2[255], output[2048], tmp[255], *ptr;
	char ctext[255];
	char gname[255];
	...
	if(argc!=2){
		printf("Funtion GE grammars parser v0.3\nUsage: %s <file>\n",argv[0]);
	...
	if ( (defunCode = retrieveCodeByName("DEFUN")) == -1) {
		printf("DEFUN token not declared, this needs to be declared where you would like your grammar to declare functions.\n");
		exit(1);
	}
	...
	if ( (funcCode = retrieveCodeByName("FUNC")) == -1) {
		printf("You need to say where functions (use FUNC) can be used (assuming a particular dna defines them).\n");
		exit(1);
	} else {
	...
	if ( (oFB = retrieveCodeByName("*(")) == -1) {
		printf("*( token not declared, apologies over this but when you use the DEFUN please define it in the grammar as \" DEFUN *( )* \" so that we can assign token numbers to the open and close brackets. [not great but not THAT inconvenient].\n");
		exit(1);
	}    
	if ( (cFB = retrieveCodeByName(")*")) == -1) {
		printf(")* token not declared, apologies over this but when you use the DEFUN please define it in the grammar as \" DEFUN *( )* \" so that we can assign token numbers to the open and close brackets. [not great but not THAT inconvenient].\n");
		exit(1);
	}   
	if ( (defunCode +1 != oFB) || (oFB +1 != cFB) ) {
		printf("WARNING - the code assumes that the following line appears early on DEFUN *( )* forming three normally consecutive terminal tokens.\n");
		printf("You will need to check code for the parse function where it removes the DEFUN symbol (and the next two) AND the case where \n");
		printf("Appears to ensure the  correct behaviour is being generated. (Or of course defune DEFUN *( )* early on.)\n");
	}
	
	if ( (commaCode = retrieveCodeByName(",")) == -1) {
		printf(", token not declared, apologies over this but when you need to use a , somewhere in the grammar as the automatic functions use commas to separate the paramaters of a function and I need lex to assign a number to the token.\n");
		exit(1);
	}    
	...
	if (symbolsTok(1, text, &code, &type, &total,text2)) {
		do {
			...
			if ( (place = chainContains(text,funcCode))) {
				printf("Found the chain %s, which contains code %d\n",text,funcCode);
				funcsThisBranch=1;
				strcpy(tmp,"_");
				strcat(tmp,text2);
				addSymbol(tmp,tmp,DUMMYNONTERMINAL);
				fprintf(f,"\t\t\tif (doingPCT2) {\n");
				fprintf(f,"\t\t\tcurr_expr[0]=FUNCPLACEHOLDER;\n");
				fprintf(f,"\t\t\tbreak;\n");
				fprintf(f,"\t\t\t}\n");
				fprintf(f,"\t\t\tif (expandPCT2) {\n");
				fprintf(f,"\t\t\t\t//cout << \"Hit a dofun with expandPCT2 true\";\n");
			...
			if ( (place = chainContains(text,puseCode))) {
				paramsThisBranch = 1;
				strcpy(tmp,"_");
				strcat(tmp,text2);
				addSymbol(tmp,tmp,DUMMYNONTERMINAL);
				...
				fprintf(f,"\t\t\t}\n");
				fprintf(f,"\t\t\tif (expandPCT2) {\n");
				fprintf(f,"\t\t\t\t//cout << \"Hit a dopuse with expandPCT2 true\";\n");
				fprintf(f,"\t\t\t\tballoonExpansion++;\n");
				fprintf(f,"\t\t\t\tgrowing=true;\n");
				fprintf(f,"\t\t\t\tmaxDepth=2;\n");
				fprintf(f,"\t\t\t\tmaxGrow = false;\n");
			...
			if (paramsThisBranch) { 
				fprintf(f,"\t\t\tdivisor = %i;\n",total-2); // we have two "defaults defined" so subtract two.
