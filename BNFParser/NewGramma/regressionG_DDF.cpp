...
regressionG_DDF *regressionG_DDF::_instance = 0;


regressionG_DDF::regressionG_DDF() { 
          allowFunctions = true;
...
regressionG_DDF* regressionG_DDF::Instance() {
          if (_instance == 0) {
                    _instance = new regressionG_DDF();
          }
          return _instance;
}


bool regressionG_DDF::doACreate(int *exp,int *length,int maxExp,long codon_list[],
...
bool regressionG_DDF::ptc2Grow(int *exp, int *length, int maxExp, long codon_list[],
...
void regressionG_DDF::ptc2GrowOne(bool allowExpand) {
...
bool regressionG_DDF::traverseParseTree(parseTreePtr cNode) {
...
void regressionG_DDF::AllowFunctions(bool al) {
          allowFunctions = al;
          if (al) cout << "Functions are allowed.\n";
          else cout << "Functions are prohibited\n";
}

bool regressionG_DDF::parse(int *exp,int *length,
...
bool regressionG_DDF::doParse(int &cUsed) {
...
bool regressionG_DDF::dealtWithPop(int expr[],int *curr,int *length) {
...
void regressionG_DDF::changeCodonsForGrow(int divisors) {
...
void regressionG_DDF::read_func() {
...
void regressionG_DDF::apply_one_grammar(int curr_expr[],int *curr,int *length)
{
			...
			switch(currentCodon%divisor){
				default:{ // We are going to call a function
					...
					*length = *length+shift;
					...
					if (fPars > 0) {
						curr_expr[*curr+2]=FPARS; // FPARS = 1005
						for (int j=0;j<fPars-1;j++) {
							curr_expr[*curr+3+2*j]=COMMA; // , = 0
							curr_expr[*curr+4+2*j]=FPARS; // FPARS = 1005
						}
					}
...
/***************************************************************/
void regressionG_DDF::print_grammar_individual(ostream& output_stream,int curr_expr[],int length){
	for(int i=0;i<length;i++)
		...
		case XVAL: {output_stream << " XVAL ";break;}
