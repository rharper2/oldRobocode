...
// %%
/***************************************************************/
using namespace regression_namespace;

regression *regression::_instance = 0;
...
regression* regression::Instance() {
          if (_instance == 0) {
                    _instance = new regression();
          }
          return _instance;
}


bool regression::doACreate(int *exp,int *length,int maxExp,long codon_list[],
...
bool regression::ptc2Grow(int *exp, int *length, int maxExp, long codon_list[],
...
void regression::ptc2GrowOne(bool allowExpand) {
...
bool regression::traverseParseTree(parseTreePtr cNode) {
...
void regression::AllowFunctions(bool al) {
          allowFunctions = al;
          if (al) cout << "Functions are allowed.\n";
          else cout << "Functions are prohibited\n";
}

bool regression::parse(int *exp,int *length,
...
bool regression::doParse(int &cUsed) {
...
bool regression::dealtWithPop(int expr[],int *curr,int *length) {
...
void regression::changeCodonsForGrow(int divisors) {
...
void regression::apply_one_grammar(int curr_expr[],int *curr,int *length)
{
...
/***************************************************************/
void regression::print_grammar_individual(ostream& output_stream,int curr_expr[],int length){
