...
#include "spatial.h"
#include "spatialRobocodeCreatureBeta.h"
...
void changeDirectory() {
	...
	if (currentGen != 0) {
		system("tar -czf robotsAdvanced.tgz *.java *.class");
...
void doit() {
 ...
 // This is the file the population is saved at, (in each second generation folder)
	const char *fileToUse = "TestOneRoboCodeBeta";
	...
	aWorld->loadPop("TestOneRoboCodeBetaGen131.pop");
	changeDirectory();
	initialiseServerStuff(9000);
	
	for (currentGen=132;currentGen<GensToDo;currentGen++) {
		changeDirectory();
		saveEm(aWorld.get());
		cout << "Calc\n";
		aWorld->calcScore();
