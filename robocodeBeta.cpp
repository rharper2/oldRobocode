...
// %%
/***************************************************************/
using namespace robocodeBeta_namespace;

robocodeBeta *robocodeBeta::_instance = 0;


robocodeBeta::robocodeBeta() { 
	allowFunctions = true;
...
robocodeBeta* robocodeBeta::Instance() {
	if (_instance == 0) {
		_instance = new robocodeBeta();
	}
	return _instance;
}
...
void robocodeBeta::ptc2GrowOne(bool allowExpand) {
...
bool robocodeBeta::traverseParseTree(parseTreePtr cNode) {
...
void robocodeBeta::AllowFunctions(bool al) {
	allowFunctions = al;
...
bool robocodeBeta::dealtWithPop(int expr[],int *curr,int *length) {
...
void robocodeBeta::changeCodonsForGrow(int divisors) {
...
void robocodeBeta::apply_one_grammar(int curr_expr[],int *curr,int *length)
{
			...
			switch(currentCodon%divisor){
				case(0):{
					...
					*length = *length + 20;
					...
					//curr_expr[*curr+6]=STATIC; //INMAINLOOP
					//curr_expr[*curr+7]=STACK;	//INMAINLOOP
					//curr_expr[*curr+8]=LOCALSTACK1; //INMAINLOOP
					//curr_expr[*curr+9]=COMMA; //INMAINLOOP
					//curr_expr[*curr+10]=LOCALSTACK2; //INMAINLOOP
					//curr_expr[*curr+11]=SEMICOLON; //INMAINLOOP
			...
			switch(currentCodon%divisor){
				case(0):{
					...
					*length = *length + 11;
					...
					//curr_expr[*curr+6]=STATIC; //static
					//curr_expr[*curr+7]=SCANNEDROBOTEVENT; //ScannedRobotEvent
					//curr_expr[*curr+8]=PE; //pe
					//curr_expr[*curr+9]=SEMICOLON; //;
					//curr_expr[*curr+10]=N; //N
			...
			switch(currentCodon%divisor){
				case(0):{
					...
					*length = *length + 11;
					...
					curr_expr[*curr+5]=N; //N
					//curr_expr[*curr+6]=STATIC; //static
					//curr_expr[*curr+7]=HITBYBULLETEVENT; //HitByBulletEvent
					//curr_expr[*curr+8]=PE; //pe
					//curr_expr[*curr+9]=SEMICOLON; //;
					//curr_expr[*curr+10]=N; //N
					curr_expr[*curr+7]=ONSTART; //ONSTART
			...
			switch(currentCodon%divisor){
				case(0):{
					...
					*length = *length + 11;
					...
					curr_expr[*curr+5]=N; //N
					//curr_expr[*curr+6]=STATIC; //static
					//curr_expr[*curr+7]=HITROBOTEVENT; //HitRobotEvent
					//curr_expr[*curr+8]=PE; //pe
					//curr_expr[*curr+9]=SEMICOLON; //;
					//curr_expr[*curr+10]=N; //N
					curr_expr[*curr+7]=ONSTART; //ONSTART
			...
			switch(currentCodon%divisor){
				case(0):{
					...
					*length = *length + 11;
					...
					curr_expr[*curr+5]=N; //N
					//curr_expr[*curr+6]=STATIC; //static
					//curr_expr[*curr+7]=HITWALLEVENT; //HitWallEvent
					//curr_expr[*curr+8]=PE; //pe
					//curr_expr[*curr+9]=SEMICOLON; //;
					//curr_expr[*curr+10]=N; //N
					curr_expr[*curr+7]=ONSTART; //ONSTART
			...
			switch(currentCodon%divisor){
				...
				case(7):{
					for(int i=*length-1;i>*curr;i--)
						curr_expr[i+1]=curr_expr[i];
					*length = *length + 1;
					curr_expr[*curr+0]=GETBATTLEFIELDHEIGHT__; //getBattleFieldWidth__
					curr_expr[*curr+1]=POPTREE;
					if (ExtractingGrammarInfo) stack.push_back(codonPos);
					break;
				}
...
/***************************************************************/
void robocodeBeta::print_grammar_individual(ostream& output_stream,int curr_expr[],int length, char *name, char *gen){
	int indent = 0;
	for(int i=0;i<length;i++)
		switch(curr_expr[i]){
			...
			case DOVARIABLEDEFS: {output_stream << "  Stack globalStack1, globalStack2;\n " <<	
				   "   Stack mlLocalStack1,mlLocalStack2;\n    Stack ohrLocalStack1,ohrLocalStack2;\n    Stack ohbbLocalStack1,ohbbLocalStack2;\n    Stack osrLocalStack1,osrLocalStack2;\n    Stack ohwLocalStack1,ohwLocalStack2;\n"
			...
			case THESTACKDOTPUSH_OB: {output_stream << " thestack.push( ";break;}
