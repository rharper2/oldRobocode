...
void setUpBattleFront() {
	if (battlers.size() %2 != 0 ) { // we have an odd number
		...
		if (verbose) cout << "A freepass granted to " << battlers[freepass] << endl;
...
//	int size = battlers.size();
//	int top = size-1;
//	int bottom = 0;
//	for (int i=0;i<size/2;i++) {
	
//	<bvector<string>::iterator at;
//	at=battlers.begin();
//	while (at!=battlers.end()) {
//		battlePtr nbp(new battle(*at++,*at++));
//		battlePtr nbp(new battle(battlers[top--],battlers[bottom++]));
//		battleFront.push_back(nbp);
						...
						{ // okay looks like we have a score
							if ((found = sockfdToCritter.find(fd)) == sockfdToCritter.end()) {
								...
								if ((parasiteScore == 0) || (creatureScore == 0)) {
									...
									if (parasiteScore == 0) { parasiteScore = 1; creatureScore = 2; }
									else { parasiteScore = 2; creatureScore = 1; }
								}
								...
								if (verbose) cout << "Battle between " << battleResult->c1 << " and parasite " << battleResult->c2 << " scored " << creatureScore << " v" << parasiteScore << endl;
								sockfdToCritter.erase(found);
								readyForWork.push_back(fd);
...
int main (int argc, char * const argv[]) {
	...
	savefile.open("top18winnersrounds.csv");
		for (theRound=0;theRound < NUMBEROFGOES;theRound++) {
			cout << "Round" << theRound << ",\n";
			verbose = false;
			savefile << "Round" << theRound << ",\n";
			for (int i=0;i<4;i++) {
				cout << savedWinners[theRound][i] << ",\n";
				savefile << savedWinners[theRound][i] << ",\n";
			}
		}
