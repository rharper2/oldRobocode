...
void saveEm(theWorld *thePtr) {
	...
	while (vat != thePtr->theWorld.end()) {
		...
		if ((*vat)->parasite->getParasite()->isValid()) {
			...
			ofstream robotout;
			robotout.open(jname);
			robocodeBeta::Instance()->print_grammar_individual(robotout,(*vat)->parasite->getParasite()->getExpression(),(*vat)->parasite->getParasite()->getExpressionLength(),fname,gname);
			robotout.close();
		} else {
...
void doit() {
	...
	const char *fileToUse = "TestOneRoboCode";
	...
	//aWorld->loadPop("TestOneRoboCode");
	aWorld.reset(spatialWorld<spatialRobocodeCreature,spatialRobocodeParasite>::makeWorldFromLoadFile(SizeofX,SizeofY,3,fileToUse));
	changeDirectory();
	initialiseServerStuff(9000);
	
	for (;currentGen<GensToDo;currentGen++) {
		changeDirectory();
		saveEm(aWorld.get());
		cout << "Calc\n";
		aWorld->calcScore();
